...
main()
{
      ...
      unsigned long int i, point,size,n0=0,n1=0,n2=1;
      unsigned char stringa[5];
      
      rsb=fopen("coolres.rsb","r+b");
      jrsb=fopen("coolres0.rsb","r+b");
      mkdir("coolres0");
      chdir("coolres0");
      for (i=0;i<142;i++)
      {
          ...
          fread(&point,4,1,rsb);
          fread(&size,4,1,rsb);
          size=Endian_Int_Conversion(size);
          point=Endian_Int_Conversion(point);
          size=size-0xC0000000;
         // size=Endian_Int_Conversion(size);
          printf("%x   %x    %x\n",ftell(rsb),point,size);
          fseek(jrsb,point,SEEK_SET);
          fread(buffer,size,1,jrsb);
