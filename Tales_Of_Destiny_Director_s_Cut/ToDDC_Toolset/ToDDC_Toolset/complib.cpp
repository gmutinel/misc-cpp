...
int Decode(int version, void *in, int inl, void *out, int *outl) {
	...
	int error = SUCCESS;
	inst = (insp = (unsigned char *)in) + inl; 
	oust = (ousp = (unsigned char *)out) + *outl;
		...
		if (((flags >>= 1) & 0x100) == 0) 
{
 get(c);
 if (profilef) 
	 fprintf(profilef, "-------- %02X -------- [%08X:%08X]\n", c, insp - (unsigned char *)in, ousp - (unsigned char *)out);
 flags = c | 0xff00;
 }
		if (flags & 1) 
{
 ...
 j = (j & 0x0f) + T;
		...
		if (version == 1 || j < (unsigned int)(F)) 
{
 if (profilef) 
	 fprintf(profilef, "WINDOW[%03X,*%02X] : (", j + 1, i);
 for (k = 0;
 k <= j;
 k++) 
{
 ...
 } else 
{
 ...
 i = (i >> 8) + T;
		...
		if (profilef) 
			fprintf(profilef, "REPEAT[%03X,%02X] : (", i + 1, j);
 for (k = 0;
 k <= i;
 k++) 
{
...
_cleanup:
	...
	if (insp != inst) {
		fprintf(stderr, "insp != inst (%d != %d)", insp, inst);
